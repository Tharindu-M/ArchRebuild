package app.example.archrebuild.data

import app.example.archrebuild.data.source.local.FakeTaskDao
import app.example.archrebuild.data.source.network.FakeNetworkDataSource
...
class DefaultTaskRepositoryTest {
    ...
    fun getTasks_repositoryCachesAfterFirstApiCall() = testScope.runTest {
        ...
        val initialTasks = taskRepository.getTasks(true)

        // Change the network source
        networkDataSource.tasks = networkTasks.toMutableList()

        // Load the updated tasks from the repository
        val secondTasks = taskRepository.getTasks()

        // Initial and second should match because we didn't force a refresh
        assertThat(secondTasks).isEqualTo(initialTasks)
    }
