package app.example.archrebuild.data.source.local

import app.example.archrebuild.data.local.LocalTask
import app.example.archrebuild.data.local.TaskDao
...
class FakeTaskDao(initialTasks: List<LocalTask> = emptyList()) : TaskDao {
    ...
    override suspend fun deleteById(taskId: String): Int {
        return if (_tasks?.remove(taskId) != null) 1 else 0
    }
